...
features1 = df.drop(['event','selfcontrol','independ','novator','gender'  ],axis=1)
...
# BNB=BernoulliNB()
BNB=BernoulliNB(alpha=0.5,force_alpha=False)

# fit the model on the training data
data_fit1=BNB.fit(X_train, y_train)
...
score2=BNB.score(X_test, y_test)
...
accuracy_score_train = BNB.score(X_train, y_train)
...
BNB.fit(X_train_min_max, y_train)
train_predict_min_max =BNB.predict(X_train_min_max)
...
print(class_report_train)
accuracy_score_train_minmax = BNB.score(X_train_min_max, y_train)
...
train_predict_standard =BNB.predict(X_train_std)
...
accuracy_score_train_std = BNB.score(X_train_std, y_train)
